      ...
      <div className="p-4 border-b border-slate-800 flex flex-wrap items-center justify-between gap-4 bg-slate-900">
        ...
        <div className="flex items-center gap-3">
          ...
          <input
            ...
            className="bg-slate-800 border border-slate-700 rounded-lg px-3 py-1 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-blue-500"
          />
        </div>
      </div>

      {/* Legend */}
      <div className="px-4 py-2 border-b border-slate-800/80 bg-slate-950/40 flex flex-wrap items-center gap-4 text-xs">
        ...
        <div className="flex items-center gap-1.5 border-l border-slate-700 pl-3">
          <span className="px-1.5 py-0.5 rounded bg-purple-500/20 text-purple-300 border border-purple-400/50 text-[10px] font-bold flex items-center gap-1">
            <Layers className="w-3 h-3" /> JOINT BLOCK
          </span>
          <span className="text-slate-400 text-[11px]">Consolidated multi-dept window</span>
        </div>
        <div className="flex items-center gap-1.5 border-l border-slate-700 pl-3">
          <span className="px-1.5 py-0.5 rounded bg-yellow-500/20 text-yellow-300 border border-yellow-400/50 text-[10px] font-bold flex items-center gap-1">
            <Lock className="w-3 h-3" /> PINNED
          </span>
          <span className="text-slate-400 text-[11px]">Human Officer Lock</span>
        </div>
      </div>
        ...
        <table className="w-full text-left border-collapse min-w-[900px]">
          <thead>
            <tr className="border-b border-slate-800 bg-slate-900/80 text-[11px] font-semibold text-slate-400 uppercase tracking-wider">
              <th className="p-3 w-64 sticky left-0 bg-slate-900 z-10">Corridor Section</th>
              {distinctDates.map((date) => (
                <th key={date} className="p-3 text-center border-l border-slate-800/80 min-w-[140px]">
          ...
          <tbody className="divide-y divide-slate-800/60 text-xs">
            {sections.map((sec) => (
              <tr key={sec.section_id} className="hover:bg-slate-800/20 transition-colors">
                ...
                <td className="p-3 sticky left-0 bg-slate-900/95 z-10 border-r border-slate-800">
